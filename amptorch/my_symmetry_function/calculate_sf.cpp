...
extern "C" int calculate_sf(double** cell, double** cart, double** scale,
	...
	// V = |det [a b c]|, (a b c =cell[0] cell[1] cell[2])
    for (int i=0; i<3; ++i) {
        tmp = 0;
        for (int j=0; j<3; ++j) {
        ...
        nbins[i] = ceil(plane_d[i]/cutoff);  // plane_d[i] ????
        total_bins *= nbins[i];
    }
	// devide cell into bin
	// nbins (3) refers to number of bins in a directory
	...
	// bin_i[i][j] refers to the (fraction * bin_number) of atom i in directory j   approximately equal to cart / cutoff
	// bin_i[i][3] refers to number of bins assigned ????????
	// atoms_bin[jj]: number of atoms in bin jj? 
    max_atoms_bin = 0;
    for (int i=0; i < total_bins; ++i) {
